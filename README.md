...
When `auto-fix-command` is set, an `auto-fix` job runs after the format and lint jobs complete — even when either fails, so it can fix the files that caused the failure — and commits the fixes back to the branch with a `[skip ci]` message. The job runs with `contents: write` permission to push the fix commit.
