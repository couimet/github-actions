...
> ```
>
> See [Versioning](#versioning) for the full rationale.
...
Lints Markdown files with [markdownlint-cli2](https://github.com/DavidAnson/markdownlint-cli2) at a pinned npm version. The action honors the consuming repo's own `.markdownlint-cli2.*` / `.markdownlint.*` config via markdownlint-cli2 auto-discovery (searched in or under the working directory). When the repo has no such config, the action falls back to the canonical [`@couimet/markdownlint-config`](https://www.npmjs.com/package/@couimet/markdownlint-config) (an npm dependency of the action, Dependabot-tracked), which enforces MD060 table-column style `aligned` instead of cli2's built-in defaults. To opt out of the fallback or extend the shared defaults, add your own `.markdownlint-cli2.*` / `.markdownlint.*` config extending `@couimet/markdownlint-config`; your config stays the authority. MD060 `aligned` violations are not auto-fixable by `markdownlint-cli2 --fix`, so in a configless repo `mode: fix` reports them but cannot repair the tables. In the default `check` mode the step fails on any lint error; set `mode: fix` to run `markdownlint-cli2 --fix` and fix issues in place instead.

In `fix` mode the action auto-registers the custom rule [markdownlint-rule-force-align-table-columns](https://www.npmjs.com/package/markdownlint-rule-force-align-table-columns) (`MD060A`) when the effective config is an eligible JSON or JSONC file. JS-module configs, unparseable configs, and unresolved-rule fallbacks do not register the rule. A consumer config that extends [`@couimet/markdownlint-config`](https://www.npmjs.com/package/@couimet/markdownlint-config) can be just `{ "config": { "extends": "@couimet/markdownlint-config" } }` and `--fix` will still re-align tables. The rule is injected into a temporary effective config written next to the consumer's config; the source config is never modified. Check mode is unaffected, because `MD060A` reports on every misaligned table even when `MD060` is not configured.

| Input                  | Required | Default   | Description                                                                                                                                                                                                                                                                                              |
| ---------------------- | -------- | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `config`               | no       | (empty)   | Path to a config file passed as `--config`. When empty, the repo's own `.markdownlint-cli2.*` / `.markdownlint.*` config is the authority via auto-discovery (in or under the working directory, supports split configs); with no repo config, falls back to the bundled `@couimet/markdownlint-config`. |
...
When `auto-fix-command` is set, an auto-fix step runs after format and lint when either fails and commits the fixes back to the branch; the step only runs when the format or lint step itself failed, so unrelated build, test, or setup failures never trigger it. The consuming job must check out the PR head ref with `persist-credentials: true` and grant `contents: write` in its `permissions:` block. The step only runs on same-repository `pull_request` events, so fork PRs are skipped.
...
When `auto-fix-command` is set, an `auto-fix` job runs when format or lint fails, so it can fix the files that caused the failure, and commits the fixes back to the branch. By default the fix commit uses `chore: auto-fix [skip ci]` and is pushed with GITHUB_TOKEN, so no checks report on the fixed SHA and repos with required status checks stay merge-blocked until another commit lands. For the opt-in mode, pass the `auto-fix-token` secret (a PAT or GitHub App token) plus an `auto-fix-commit-message` without `[skip ci]`: the fix commit then re-triggers the pipeline so checks report green on the fixed SHA, while the auto-fix job skips its own commits by matching the head commit subject against the configured `auto-fix-commit-message`, and git-auto-commit-action no-ops when there is nothing to fix. The `auto-fix` job declares no static `permissions`: it inherits the calling job's grant, so read-only callers are unaffected and auto-fix only pushes when the caller opts in. To enable auto-fix, grant `contents: write` on the calling job's `permissions` and pass `auto-fix-command`; callers that stay on `contents: read` need no per-repo change.

| Input                      | Required | Default                     | Description                                                                                                                                                                                                                     |
| -------------------------- | -------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `auto-fix-command`         | no       | (empty)                     | Command to run for auto-fixing after format and lint (e.g., `pnpm format:fix && pnpm lint:fix`). When empty (the default), auto-fix is disabled and the job is skipped; when set, the calling job must grant `contents: write`. |
...
To enable auto-fix, grant `contents: write` on the calling job and pass `auto-fix-command`:

```yaml
jobs:
  ci:
    permissions:
      contents: write # auto-fix opt-in; otherwise keep contents: read
      pull-requests: write
    uses: couimet/github-actions/.github/workflows/typescript-ci-checks.yml@main
    with:
      working-directory: .
